 ...
 * @param {Object} options    - Perform specific tasks based on input flags.
 *
 * @returns {string} A URL to use in a HTTP Request
 */
 api.urlFor = function(resource, id, parameters, options) {
  ...
  if (typeof options !== "undefined" && typeof options.convertMultiParamsToArray !== "undefined" && options.convertMultiParamsToArray) {
    _.each(parameters, function(value, key) {
        if (Array.isArray(value)) {
          url = url.split(key).join(key + "[]");
        }
    });
  }
 ...
 * @param {object} options    - Key-value based option flags.
 *
 * @returns {Object} an array of items for the specified resource.
 */
 api.index = function(resource, parameters, options) {
  return this.getTokens().then(_.bind(function(tokens) {
    var deferred = q.defer();
    var req = {url: this.urlFor(resource, null, parameters, options), headers: {'Authorization': 'Bearer ' + tokens.access_token}, json: true};
 ...
 * @param {Object} parameters - The query string parameters to include with the request.
 */
api.indexAll = function(resource, parameters, pageSize) {
  if (!pageSize) {
    pageSize = this.settings.maxLimit;
  }

  var promises = [];
  promises.push(this.index(resource, _.extend({}, parameters, {offset: 0, limit: pageSize})));
  return promises[0].then(_.bind(function(firstPage) {
    for (var offset = pageSize; offset < firstPage.body.pagination.total; offset += pageSize) {
      promises.push(this.index(resource, _.extend({}, parameters, {offset: offset, limit: pageSize})));
    }
...
/**
 * Creates an item for the specified resource from the provided parameters using form data.
 *
 * @param {string} resource   - The resource name the item is being created with.
 * @param {Object} parameters - The body of the request containing the details of the item to be created.
 *
 * @returns {Promise} A Promise to return an object containing the id for the resource created.
 */
api.postFormData = function(resource, parameters) {
  var deferred = q.defer();
  ...
  return this.getTokens().then(_.bind(function(tokens) {
    var req = {
      ...
      method: 'POST',
      formData : parameters,
      headers: {'Authorization': 'Bearer ' + tokens.access_token, "Content-Type": "multipart/form-data"}
    };
...
function resolveResponse(deferred, api, req) {
  return function(error, res, body) {
    if (info.enabled && req && req.url && req.timingPhases) {
      info("FINISHED %s %ims", req.url, res.timingPhases.total);
    }
...
/**
 * Get: API Methods that do not require tokens but run because ip is on Trader Interactive network.
 * @param {string} resource   - The resource name to use.
 * @param {string} id         - The id to be provided toteh resource endpoint.
 * @param {Object} parameters - The query string parameters to use in the request.
 *
 * @returns {Object} The item with the specified id.
 */
api.getByNetworkCheck = function(resource, id, parameters) {
  var deferred = q.defer();
  if (!id) {
    deferred.reject({error: {message: "An id is required"}});
    return deferred.promise;
  ...
  var req = {url: this.urlFor(resource, id, parameters), json: true};
...
/**
 * Index: API Methods that do not require tokens but run because ip is on Trader Interactive network.
 *
 * @param {string} resource   - The name of the resource to send the request to.
 * @Param {Object} parameters - The query string parameters to include with the request.
 *
 * @returns {Object} an array of items for the specified resource.
 */
api.indexByNetworkCheck = function(resource, parameters) {
    var deferred = q.defer();
    var req = {url: this.urlFor(resource, null, parameters), json: true};
...
/**
 * Post: API Methods that do not require tokens but run because ip is on Trader Interactive network.
 *
 * @param {string} resource   - The resource name the item is being created with.
 * @param {Object} parameters - The body of the request containing the details of the item to be created.
 *
 * @returns {Promise} A Promise to return an object containing the id for the resource created.
 */
api.postByNetworkCheck = function(resource, parameters) {
  var deferred = q.defer();
  var req = {
    url: this.urlFor(resource),
    method: 'POST',
...
/**
 * Put: API Methods that do not require tokens but run because ip is on Trader Interactive network.
 *
 * @param {string} resource   - The resource name to up update the item.
 * @param {string} id         - The unique identifier of the item to be updated.
 * @param {Object} parameters - The information to update the item with.
 ...
 * @returns {Promise} A Promise to return an object confirming the item was updated.
 */
api.patchByNetworkCheck = function(resource, id, parameters) {
  var deferred = q.defer();
...
/**
 * Delete: API Methods that do not require tokens but run because ip is on Trader Interactive network.
 *
 * @param {string} resource - The resource name to send the request to.
 * @param {string} id       - The unique identifier for the item to be deleted.
 *
 * @returns {Promise} A Promise to return an object confirming the item was deleted.
 */
api.deleteByNetworkCheck = function(resource, id) {
  var deferred = q.defer();
